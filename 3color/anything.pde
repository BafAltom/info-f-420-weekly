...
Pfont font;
ArrayList<ArrayList<PVector> > coloredPoints = new ArrayList<ArrayList<PVector> >();
coloredPoints.add(new ArrayList<PVector>());
coloredPoints.add(new ArrayList<PVector>());
coloredPoints.add(new ArrayList<PVector>());
...
void printTriangle(ArrayList<PVector> tri)
{
    String s = "tri\t";
    for (PVector p : tri) {
        s += "(" + str(p.x) + ", " + str(p.y) + ")";
...
void draw()
{
    ...
    // find color with less points
    ArrayList<PVector> minColor = coloredPoints.get(0);
    for (int i = 1; i < 3; i++) {
        if (coloredPoints.get(i).size() < minColor.size()) {
            minColor = coloredPoints.get(i);
        }
    }
    // draw it
    for (p : minColor) {
...
{
    if (mouseButton == LEFT) {
        for (ArrayList<PVector> colorSet : coloredPoints) {
            colorSet.clear();
        }
...
*/
{
    ...
    triangles.add(pointsCopy);
    // color the last three points
    coloredPoints.get(0).add(pointsCopy.get(0));
    coloredPoints.get(1).add(pointsCopy.get(1));
    coloredPoints.get(2).add(pointsCopy.get(2));
    // color the other points in reverse order
    for (int i = triangles.size() - 2; i >= 0; i--) {
        // find the color of the previous point of the ear
        // This code is hideous because I did not want to create a ColoredPoint class
        ArrayList<PVector> tri = triangles.get(i);
        int colorPre = -1;
        int colorNex = -1;
        for (int c = 0; c < 3; c++) {
            ArrayList<PVector> colorSet = coloredPoints.get(c);
            if (colorSet.contains(tri.get(0))) {
                colorPre = c;
            }
            if (colorSet.contains(tri.get(1))) {
                colorNex = c;
            }
        }
        int colorP = 3 - (colorPre + colorNex)
        coloredPoints.get(colorP).add(tri.get(2));
    }
    return triangles;
}
